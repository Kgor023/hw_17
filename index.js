...
//задание 2
var userText = prompt('Введиет строчку/текст');
function getText(){
...
var userPrefer = prompt('Какой фрукт вы предпочитаете?');
function getFruits(list){
...
function getSum(secondParam) {
    var sum = 0;
    for (var i = 0; i <= secondParam; i++) {
        sum = sum + i;
    }
    return sum;
}
...
function getCredit(creditSum) {
    var rate = 0.17 / 100 / 12;
    var monthlyPayment = creditSum * (rate * ((1 + rate) ** 60)) /(((1 + rate) ** 60) - 1) ;
    var overpatment = monthlyPayment * 60 - creditSum;
    return overpatment;
